...
function ecoGetOldDocument(xhr){
	...
	var ok = true;
	var tagPag = ';'
	var visible = vgk.appPagTopols.visible;
	switch (visible){
		case 'CLASES' :
...
function grabaTxtML(nodo){
	...
	if (vgk.appModal.editON  && vgk.appPagTopols.visible == 'MENUS'){ 
			vgk.menuML.updtNodoSelf(nodo);
			updateMenuML();
		}
	else if (vgk.appModal.editON  && vgk.appPagTopols.visible == 'TEXTOS'){
			vgk.textosML.updtNodoSelf(nodo);
			updateTextosML();
		}
	else if (!vgk.appModal.editON  && vgk.appPagTopols.visible == 'MENUS'){ 
...
function borraTxtML(nodo){
	...
	if (vgk.appPagTopols.visible == 'MENUS'){ 
			vgk.menuML.borraNodo(nodo);
			updateMenuML();
		}
	else if (
		vgk.appPagTopols.visible == 'TEXTOS'){
...
function showClasesML(){
	...
	vgk.appPagTopols.setLangs(codes);
	vgk.appPagTopols.setNodos(nodos);
}
...
function ecoGetClasesML(xhr){
	...
	if (items.length > 0) {
//		vgk.appPagTopols.setLista(items);
...
function adminClasesML(){
	vgk.appPagTopols.visible = 'CLASES';
	vgk.appPagTopols.malla = [];
	ajaxGetClasesML();
}

function adminMenusML(){
	vgk.appPagTopols.visible = 'MENUS';
	vgk.appPagTopols.malla = [];
	ajaxGetMenusML();
}
...
function adminTopolsML(){
	vgk.appPagTopols.visible = 'TOPOLS';
	vgk.appPagTopols.malla = [];
	ajaxGetTopolsML();
}
...
function ecoGet1MenuML(xhr){
	...
	vgk.appPagTopols.malla = vgk.menuML.getMatriz();
	vgk.appPagTopols.menuML = vgk.menuML.meta.tag;
...
function ecoNouTextosML(xhr){
	...
	vgk.appPagTopols.texts.push({"_id":loTopol._id,"meta":loTopol.meta});
	vgk.appPagTopols.textML = loTopol.meta.tag;
	vgk.appPagTopols.malla = [];
...
function ecoGet1TextosML(xhr){
	...
	vgk.appPagTopols.malla = vgk.textosML.getMatriz();
	vgk.appPagTopols.textML = vgk.textosML.meta.tag;
}
...
function ecoUpdtTextosML(xhr){
	...
	vgk.appPagTopols.malla = vgk.textosML.getMatriz();
	vgk.appPagTopols.textosML = vgk.textosML.meta.tag;
}
...
function ecoGet1TopolsML(xhr){
	...
	vgk.appPagTopols.metaML = tag;
	r$('topolJSON').value = JSON.stringify(loTopol,null,'\t'); //respTxt;
}
...
function ecoUpdtTopolsML(xhr){
	...
	vgk.appPagTopols.malla = vgk.topolsML.getMatriz();
	vgk.appPagTopols.textosML = vgk.topolsML.meta.tag;
}
...
function initAppsTopols(){

	if (r$('pagTopols')){
		vgk.appPagTopols = new Vue ({
			el: '#pagTopols',
			data : {
				keo : 'ES',
				langs : [],
...
//------------------------------------------------------------------- Textos Pag
// Definir appPagTopols DESPUES de aplicar traducciones !!!!!
...
function ecoGetTextPag(xhr){
	...
	if (items.length) get1TextPag(items[0]._id);
		...
		initAppsTopols();  // Y aquí si no hay textos ------------
		adminTopolsML();
	}
}
...
function sesionTopolsOK(xhr){
	initAppsGlobal();
//	vgk.user.keo='CAT';
	ajaxGetMenuPag('Topologias');
	ajaxGetTextPag('Topologias');
}
...
function initTopols(){
	validaSesion('usrMenu', sesionTopolsOK); // libK1_sesion.js
}
